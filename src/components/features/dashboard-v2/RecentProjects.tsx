    ...
    <GlassPanel className="flex h-full flex-col gap-3 p-4">
      ...
      {displayProjects.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-3">
          <FolderPlus className="h-8 w-8 text-white/20" />
          <span className="text-xs text-white/40">No projects yet</span>
          <button
            onClick={() => router.push('/dashboard/projects?new=1')}
            className="rounded-full bg-white/10 px-4 py-1.5 text-xs font-medium text-white/70 transition-colors hover:bg-white/15 hover:text-white"
          >
          ...
          {displayProjects.map((project, i) => (
            ...
            >
              ...
              <div className="rounded bg-white/[0.06] px-2.5 py-1">
                <span className="font-mono text-[13px] font-bold text-white/[0.87]">
                  {project.photos}
                </span>
              </div>
