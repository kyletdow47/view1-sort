...
const STATUS_STYLES: Record<string, string> = {
  Paid: 'bg-emerald-400/10 text-emerald-400 border border-[#95d1d1]/20',
  Pending: 'bg-primary/10 text-primary border border-primary/20',
  Refunded: 'bg-[#ffb4a5]/10 text-rose-400 border border-[#ffb4a5]/20',
}
    ...
    <div className="min-h-screen bg-background text-on-surface">
      <div className="max-w-[1200px] mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          {/* Total Revenue Card */}
          <div className="lg:col-span-8 bg-surface rounded-xl border border-outline-variant/40 p-6">
          ...
          {/* Current Plan */}
          <div className="lg:col-span-4 bg-surface rounded-xl border border-outline-variant/40 p-6 flex flex-col">
            <p className="text-[10px] uppercase tracking-widest font-medium text-on-surface-variant mb-4">
              Current Plan
            </p>
            ...
            <p className="text-xs text-on-surface-variant mt-2 flex-1">{plan.description}</p>
            <Link
              href="/dashboard/settings"
              className="mt-4 flex items-center justify-center gap-2 rounded-lg border border-outline-variant px-4 py-2.5 text-sm font-medium text-on-surface-variant hover:bg-surface-container transition-colors"
            >
              Manage Plan
        ...
        <div className="bg-surface rounded-xl border border-outline-variant/40 p-6">
          ...
          {connectStatus === 'not_connected' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-surface-container rounded-xl p-5">
                <p className="text-[10px] uppercase tracking-widest font-medium text-on-surface-variant mb-2">
                  Available Balance
                </p>
              ...
              <div className="bg-surface-container rounded-xl p-5 flex flex-col justify-between">
                <p className="text-[10px] uppercase tracking-widest font-medium text-on-surface-variant mb-3">
                  Quick Actions
                </p>
                <div className="space-y-2">
                  <button className="w-full flex items-center justify-between px-4 py-2.5 rounded-lg border border-outline-variant text-sm font-medium text-on-surface-variant hover:bg-surface-container transition-colors">
                    View Payout History
                    <ChevronRight size={14} className="text-on-surface-variant" />
                  </button>
                ...
                <thead>
                  <tr className="border-b border-outline-variant/40">
                    {['ID', 'Description', 'Status', 'Amount', 'Date'].map((col) => (
                      <th
                        key={col}
                        className="text-left px-6 py-3.5 text-[10px] uppercase tracking-widest font-medium text-on-surface-variant"
                      >
                        {col}
                      </th>
                    ))}
