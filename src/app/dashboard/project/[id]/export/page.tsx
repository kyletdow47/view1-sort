      ...
      <div className="grid grid-cols-12 gap-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                >
                  ...
                  <span className={`text-sm font-medium ${format === opt.key ? 'text-primary' : 'text-on-surface-variant'}`}>
                    {opt.label}
                  </span>
                ...
                <div key={toggle.label} className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative h-6 w-11 rounded-full transition-colors ${
                      toggle.value ? 'bg-primary' : 'bg-surface-highest'
                    }`}
                  >
                    <span
          ...
          <div className="rounded-2xl border border-outline-variant/30 bg-surface p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-on-surface">
                  ...
                  <span className="text-on-surface-variant">(248 files)</span>
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2 mb-5">
              <div className="flex items-center justify-between">
                <span className="text-xs text-on-surface-variant">Ready to export</span>
                <span className="text-xs font-medium text-on-surface-variant">0%</span>
              </div>
        ...
        <div className="col-span-12 lg:col-span-5 space-y-6">
          ...
          <div className="rounded-2xl border border-outline-variant/30 bg-surface p-6 sticky top-6">
            ...
            <div className="rounded-xl bg-background border border-outline-variant/20 p-4 font-mono text-sm">
              {FOLDER_TREE.map((entry, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {entry.files > 0 && (
                      <span className="text-on-surface-variant text-xs">({entry.files} files)</span>
                    )}
                  </div>
                )
              })}
            ...
            <div className="mt-6 pt-5 border-t border-outline-variant/30">
              ...
              <div className="space-y-3">
                {PAST_EXPORTS.map((exp) => (
                  <div
                    key={exp.id}
                    className="flex items-center justify-between rounded-xl bg-background border border-outline-variant/20 p-3"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`flex items-center justify-center w-8 h-8 rounded-lg ${
                        exp.status === 'completed' ? 'bg-emerald-400/10' : 'bg-surface-highest/60'
                      }`}>
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-on-surface-variant">{exp.date}</p>
                      </div>
                    </div>
                    {exp.status === 'completed' ? (
