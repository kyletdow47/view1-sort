...
export function PaymentConfirmationEmail({
  ...
  return (
    <Html>
      <Head />
      <Preview>Payment confirmed — {amount} for {projectName}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Heading style={heading}>Payment Confirmed</Heading>
          <Text style={text}>Hi {clientName},</Text>
          <Text style={text}>
            Your payment of <strong>{amount}</strong> for <strong>{projectName}</strong> has been received.
          </Text>
          <Section style={receiptCard}>
            <Text style={receiptLine}>Project: {projectName}</Text>
            <Text style={receiptLine}>Amount: {amount}</Text>
            <Text style={receiptLine}>Status: Paid</Text>
          </Section>
          <Text style={text}>
            You now have full access to download your photos.
          </Text>
